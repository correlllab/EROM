...
def min_dist_to_mesh( q, verts ):
    """ Given a list of triangle verts, Return the least distance to the mesh, HACK: We don't actually care if it's very accurate! """
    ...
    # HACK: CHOOSE POINT DISTANCE IF OUTSIDE OF BUBBLE, CHOOSE MIN IF INSIDE BUBBLE
    dMin = 1e9
    for i in range( Npnts ):
        if (cenDs[i,0] > (diams[i]*factr)):
            dMin = min( dMin, pntDs[i,0] )
        else:
            dMin = min( dMin, min( plnDs[i,0], pntDs[i,0] ) )
    return dMin
